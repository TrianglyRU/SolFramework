...
function fade_perform_flash(_direction, _step_duration, _game_control = true, _end_routine = -1)
{
	var _speed = _step_duration > 0 ? (1 / _step_duration) : 0;
	fade_perform
	(
		_direction, _direction == FADEDIRECTION.IN ? FADETYPE.FLASHORDER : FADETYPE.FLASHSYNC, _speed, _step_duration, _game_control, _end_routine
	);
}